...
# ---------------------------------------------------------------------------- #
  ...
  character_classes = rec {
    ...
    unreserved_c = "[:alnum:]${mark_c}";  # XXX: mark_c
    res_ns_c     = ";?:@&=+$,";
    reserved_c   = res_ns_c + "/";
  ...
  patterns = with character_classes; with pseudo_ccs; rec {
    ...
    ipv6_addr_p = let
      seg = "[0-9a-fA-F:]+(%[${aldash_c}]+)?";
    in "([[]${seg}[]]|${seg})";
    top_label_p    = "[[:alpha:]]([${aldash_c}]*[[:alnum:]])?";
    domain_label_p = "[[:alnum:]]([${aldash_c}]*[[:alnum:]])?";
